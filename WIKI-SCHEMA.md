...
## Worker-managed taxonomy mirrors — the highest-leverage pattern

For every canonical taxonomy file (e.g., `business/services.md`, `business/groups.md`), the wiki MCP auto-generates a paired `<name>-actual.md` that reads frontmatter from every entry using that taxonomy and counts the actual values in use. The mirror is regenerated on a schedule (15 minutes).

This makes the **open-vocabulary-at-write, closed-taxonomy-at-registry** discipline self-auditing. The librarian eyeballs `services-actual.md` and sees:

| What's surfaced | Action |
|---|---|
| Values used 3+ times not in the canonical list | Promotion candidates — add to `services.md` |
| Canonical values with zero usage | Deprecation candidates — remove or merge |
| Data quality issues (e.g., a JSON blob serialised into a string field) | Surface to user as finding |

Borrowed directly from goanna's `business/services-actual.md` pattern (audit 2026-05-26). Each `-actual.md` file opens with:

```
<!-- auto-generated by Office Town wiki MCP — manual edits will be overwritten -->
---
auto_generated: true
last_regenerated: 2026-05-26T08:31:17Z
source: frontmatter_json from wiki/orgs/*/entity.md
taxonomy_file: wiki/business/services.md
---
```

Frontmatter explicitly carries `auto_generated: true` so agents know not to edit.

## `applies_to:` — agent-steering field

Borrowed from goanna. A freeform phrase in concept files saying when the file is relevant:

```yaml
applies_to: business, wiki/orgs/<slug>/entity.md frontmatter
```

Better than tags for "when should I read this file?" — tags are for retrieval, `applies_to:` is for steering. Used by the librarian when curating; used by other agents when deciding whether to load context.

## Status honesty on concept files

Borrowed from goanna. Be explicit about file maturity:

```yaml
status: thin       # known weakness, growth area
status: drafted    # initial pass, needs review
status: rich       # comprehensive, current
status: stale      # outdated, needs refresh
status: historic   # superseded, kept for context
```

Lets the librarian and other agents weight content by confidence. A `thin` knowledge entry counts less than a `rich` one when synthesising.
...
6. **Don't pollute frontmatter with vector metadata** (embed_text, embed_model, etc.). Keep that in D1 alongside the audit table. Frontmatter is for humans; vector metadata is for the indexer.
7. **Don't pre-enumerate every status value.** Open vocabulary first. Canonicalise via the `-actual.md` mirror once usage stabilises.
