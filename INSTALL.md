...
3. **Wire this Mac** — runs the worker's own `connect.sh`: bootstraps the Goose CLI if missing, disables Goose's built-in Memory (the `wiki` MCP replaces it), wires the `office-town-*` MCPs into `~/.config/goose/config.yaml` (six by default, plus `sandbox` when the deployment has it), installs the [officetowd](https://github.com/jezweb/officetowd) sync daemon, creates `~/OfficeTown/` and populates it from [office-town-starter](https://github.com/jezweb/office-town-starter) — the roles, buildings, wiki, and workflows all live in the town now, so there's no separate plugin to install — runs a first sync, and auto-installs the apps onto your Apps page.
